        ...
        <Card v-if="site.git_connection">
            ...
            <CardContent class="space-y-4">
                <div v-if="site.git_connection.deploy_key">
                    <p class="mb-2 text-sm text-muted-foreground">
                        Add this deploy key to your repository (read-only access
                        is enough):
                    </p>
                    <pre
                        class="overflow-x-auto rounded-md bg-muted p-4 text-xs"
                        >{{ site.git_connection.deploy_key.public_key }}</pre>
                </div>
                <div v-if="webhookUrl && webhookSecret">
                    <p class="mb-2 text-sm text-muted-foreground">
                        To push-to-deploy, add a webhook in your repository
                        settings with this payload URL and secret (content type:
                        application/json):
                    </p>
                    <pre
                        class="overflow-x-auto rounded-md bg-muted p-4 text-xs"
...
Secret: {{ webhookSecret }}</pre>
        ...
        <Card v-if="site.status === 'active' && canManageEnvironment">
            <CardHeader>
                ...
                <CardDescription
                    >The .env file deployed with this site</CardDescription
                >
            </CardHeader>
            <CardContent>
                <Form
                    v-bind="SiteEnvironmentController.update.form(site.id)"
                    class="space-y-4"
                    v-slot="{ processing }"
                >
                    <Textarea
                        ...
                        :default-value="env ?? ''"
                    />
                    <Button :disabled="processing">Save environment</Button>
                </Form>
            </CardContent>
        </Card>
    </div>
